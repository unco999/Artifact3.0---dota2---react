...
export class spell_skill{
    ...
    call_spell_skill(id:string,player:PlayerID,target:uuid,spell_ability_card_uuid:uuid,target_index?:number){
        ...
        if(abilityInstance.wounded){
            ...
            if(!(_target as Unit).isinjuried()){
                CustomGameEventManager.Send_ServerToPlayer(PlayerResource.GetPlayer(player),"S2C_INFORMATION",{"information":"该单位没有受伤"})
                return
            }
        }
